...
class UartServoManager:
    ...
    def set_servo_angle(self, servo_id, angle, interval=None, mean_dps=None, power=0):
        '''发送舵机角度控制请求
        @param servo_id 
            舵机的ID号
        @param angle 
    ...
    def set_servo_angle_v(self, servo_id:int, angle:float, velocity:float, t_acc:float, t_dec:float, power:float=0):
